...
$(document).ready(function () {
  ...
  $("#uploadUserImagesForm").on("submit", function (e) {
    e.preventDefault();
    uploadUserVerificationImages(this);
  });
});
...
function uploadUserVerificationImages(form) {
  console.log("Form submitted");
  var formData = new FormData(form);
  $.ajax({
    url: "/Shanty-Dope-Proj/CareToFund/uploadUserVerificationImages",
    method: "POST",
    data: formData,
    processData: false,
    contentType: false,
    success: function (result) {
      // console.log("AJAX response:", result);
      var editImagesModal = bootstrap.Modal.getInstance(
        document.getElementById("editImagesModal")
      );
      if (editImagesModal) editImagesModal.hide();

      var imageUploadSuccessModal = new bootstrap.Modal(
        document.getElementById("imageUploadSuccessModal")
      );
      imageUploadSuccessModal.show();
      reloadHeader();
    },
    error: function (error) {
