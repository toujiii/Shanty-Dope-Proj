...
function viewCharityRequests(sorting) {
  $.ajax({
    ...
    success: function (result) {
      ...
      filteredDatas = sortedDatas.filter((requestCharity) => {
        return Object.values(requestCharity).some((value) =>
          String(value).toLowerCase().includes(requestSearchValue)
        );
      });
        ...
        charityRequestsHTML += `
                            ...
                            <div class="container mt-2 p-0">
                                ...
                                <p class="m-0 py-3 text-decoration-underline" style="cursor: pointer; color: #1b3c53; font-size: 0.9rem; width: fit-content;"
                                    ...
                                    onclick="getCharityRequestDetails(${
                                      data.request_id
                                    }, ${data.user_id})"
                                >
                                    View More
...
function getCharityRequestDetails(requestId, userId) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      $("#id_image_details").attr(
        "src",
        "/Shanty-Dope-Proj/CareToFund/" + data[0].id_att_link
      );
      $("#front_face_image_details").attr(
        "src",
        "/Shanty-Dope-Proj/CareToFund/" + data[0].front_face_link
      );
      $("#side_face_image_details").attr(
        "src",
        "/Shanty-Dope-Proj/CareToFund/" + data[0].side_face_link
      );
    },
    error: function (error) {
      alert("Something went wrong.");
