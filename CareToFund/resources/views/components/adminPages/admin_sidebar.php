<div id="adminSidebar" class="col-auto px-0 d-block" style="background-color: #1b3c53; min-width: 200px; max-width: 200px; ">
    <div class="h-100 d-flex flex-column align-items-center py-3 pb-0 text-white">
        <div class="container" >
            <div class="container d-flex justify-content-center border-bottom border-4 border-light" style="height: 100px;">
                <img class="img-fluid" src="/Shanty-Dope-Proj/CareToFund/resources/img/website_logo.png" alt="logo">
            </div>
        </div>
        <div class="container mt-4">
            <h1 class="m-0 " style="font-size: 20px;">
                Welcome Back, Admin!
            </h1>
            <p class="m-0 mt-3" style="font-size: 0.9rem;">
                See what's waiting for you today.
            </p>
        </div>
        <div class="container mt-auto mb-0 px-0 d-flex justify-content-center">
            <form action="/Shanty-Dope-Proj/CareToFund/signOut" method="POST">
                <button class="btn btn-signout text-white" style="font-size: 1rem; border-radius: 0px; width: 200px; height: 60px;">
                    <i class="bi bi-box-arrow-left me-1"></i>
                    Sign Out
                </button>
            </form>
        </div>
    </div>
</div>